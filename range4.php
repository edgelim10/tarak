...
if (!$connectdb){
	...
	if(ISSET($_POST['search'])){
		...
		$query=mysql_query("SELECT productid, store_code, date, brand, item_name, item_code, barcode, sku, SUM(qty), unit_price FROM cart WHERE store_code='$storecode' AND date(date) BETWEEN '$date1' AND '$date2' group by productid order by brand ASC") or die(mysqli_error());
...
<?php
		...
		</tr><a href='export3.php?storecode=$storecode&date1=$date1&date2=$date2'>export</a>";
